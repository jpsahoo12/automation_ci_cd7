...
Param(
    ...
    [Parameter(Mandatory = $false)][String]$DataSourceUsername,
    [Parameter(Mandatory = $false)][String]$DataSourcePassword
)

Import-Module $PSScriptRoot\ps_modules\PowerBI -Force
Import-Module $PSScriptRoot\ps_modules\ADAL.PS -Force
...
Write-Debug "DataSourceUsername    : $($DataSourceUsername)";
Write-Debug "DataSourcePassword    : $(if (![System.String]::IsNullOrWhiteSpace($DataSourcePassword)) { '***'; } else { '<not present>'; })";
...
elseif ($Action -eq "UpdateDatasourceViaGateway") {
    Write-Host "Trying to update the datasource with GatewayInformation"

    if ($UpdateAll -eq $false -and $Dataset -eq "") {
        Write-Error "When the update all function isn't checked you need to supply a dataset."
    }
    
    Update-PowerBIDatasetDatasourcesViaGateway -WorkspaceName $WorkspaceName -DataSetName $Dataset -AccessToken $token -NewServer $NewServer -NewDatabase $NewDatabase -DataSourceUsername $DataSourceUsername -DataSourcePassword $DataSourcePassword -UpdateAll $UpdateAll
}
