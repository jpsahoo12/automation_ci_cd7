...
Function Update-PowerBIDatasetDatasourcesViaGateway {
    Param(
        ...
        [parameter(Mandatory = $false)]$DataSourceUsername,
        [parameter(Mandatory = $false)]$DataSourcePassword,
    ...
    if ($groupPath) {
        if ($UpdateAll) {
            ...
            foreach ($dataset in $datasets) {
                $dataSources = Get-DataSetDataSources -GroupPath $groupPath -AccessToken $AccessToken -DataSetId $dataset.Id
                foreach ($dataSource in $dataSources) {
                    $dataSourceDetail = Get-DataSourceFromGateway -AccessToken $AccessToken -DataSourceId $dataSource.datasourceId -GatewayId $dataSource.gatewayId
                    Update-DataSourceFromGateway -AccessToken $AccessToken -DataSourceDetail $dataSourceDetail -NewServer $NewServer -NewDatabase $NewDatabase -DataSourceUsername $DataSourceUsername -DataSourcePassword $DataSourcePassword
                }
            }
        }
        else {
            ...
            if ($dataset) {
                ...
                foreach ($dataSource in $dataSources) {
                    $dataSourceDetail = Get-DataSourceFromGateway -AccessToken $AccessToken -DataSourceId $dataSource.datasourceId -GatewayId $dataSource.gatewayId
                    Update-DataSourceFromGateway -AccessToken $AccessToken -DataSourceDetail $dataSourceDetail -NewServer $NewServer -NewDatabase $NewDatabase -DataSourceUsername $DataSourceUsername -DataSourcePassword $DataSourcePassword -Verbose
                }
            }
            else {
...
Function Update-DataSourceFromGateway {
    Param(
        [parameter(Mandatory = $true)]$AccessToken,
        [parameter(Mandatory = $false)]$DataSourceDetail,
        [parameter(Mandatory = $false)]$NewServer,
        [parameter(Mandatory = $false)]$NewDatabase,
        [parameter(Mandatory = $false)]$DataSourceUsername,
        [parameter(Mandatory = $false)]$DataSourcePassword
    )
    $GatewayId = $DataSourceDetail.gatewayId
    $DataSourceId = $DataSourceDetail.id
    $url = $powerbiUrl + "/myorg/gateways/$GatewayId/datasources/$DataSourceId"
    $datasourceType = $DataSourceDetail.datasourceType
    $connectionDetails = "{\""server\"":\""$NewServer\"",\""database\"":\""$NewDatabase\""}"
    ...
    Write-Verbose $body
    Invoke-API -Url $url -Method "PATCH" -AccessToken $AccessToken -Body $body -ContentType "application/json"
}
...
Function Get-DataSourceFromGateway {
    Param(
        [parameter(Mandatory = $true)]$AccessToken,
